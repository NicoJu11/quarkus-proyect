...
public class OrderPostServiceImpl implements OrderPostService {
  ...
  PurchaseOrderRepository orderRepository;
    ...
    @Channel("order-shipping-emitter")
    Emitter<OrderShipping> emitter;
  ...
  public Uni<Map.Entry<PurchaseOrderResponseDto, Boolean>>  create(PurchaseOrderRequestDto orderPostRequestDto) {
      ...
      return orderRepository
              ...
              .onItem().ifNull().switchTo(() ->
                      orderRepository
                              ...
                              .invoke(() -> {
                                  var orderPost = toOrderPostAvro(orderPostRequestDto);
                                  var message = Message.of(orderPost)
                                          .addMetadata(
    ...
    private OrderShipping toOrderPostAvro(PurchaseOrderRequestDto request) {
        return new OrderShipping(
                request.shipperId(),
                request.purchaseOrderId(),
                request.carrier(),
                request.trackingNumber(),
                request.amountShipper().toString(),
                request.statusId(),
                request.noteShipper()
        );
    }
